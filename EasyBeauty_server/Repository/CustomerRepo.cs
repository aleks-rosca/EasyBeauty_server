    ...
    public static class CustomerRepo
    {
        public static void CreateCustomer(Customer customer)
        {
            DBConnection.DatabaseConnection.Execute
                (@"INSERT INTO Customer(phoneNr,name, email) VALUES (@phoneNr, @name, @email)",
                    new { phonenr = customer.PhoneNr, name = customer.Name, email = customer.Email});
        }
        ...
        {
            DBConnection.DatabaseConnection.Execute
            (@"UPDATE Customer SET name = @name, email = @email WHERE phoneNr = @phoneNr", 
        ...
        public static bool CheckCustomer(int phoneNr)
        {
            return DBConnection.DatabaseConnection.ExecuteScalar<bool>
                (@"SELECT 1 FROM Customer WHERE phoneNr = @phoneNr", new { phoneNr });
        }
    }
}